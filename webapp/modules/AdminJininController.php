...
require_once _MODULE_DIR_ . 'AdminJininForm.php';

$act  = $this->request->getParameter(_REQ_ACTION_);
//$th_code = $this->request->getParameter('th_code');

//QuickForm の設定
$qform =& new AdminJininForm('AdminJininForm');
...
case 'Display':    //トークンのセット
    ...
    $this->tpl_name = 'admin/AdminJinin_Input.tpl';
...
case 'Confirm':
    if ($act != 'Confirm') {
	    ...
	    $this->tpl_name = 'admin/AdminJinin_Confirm.tpl';
	...
	//変更前のデータをDBから取得
        $thdbh->table = "sy_jinin";
        $beforeData = $thdbh->getTehaiDefault(3);
...
//    }
    ...
    if (count($diff) == 0) {
        $this->tpl_name = 'admin/AdminTehai_List.tpl';
        $this->renderer->assign('message', "人員配置の変更データはありませんでした。");
...
*/
        $this->renderer->assign('message', "人員配置の初期値を変更しました。");
        $this->tpl_name = 'admin/AdminTehai_List.tpl';
    //DBへ変更失敗した場合
    } else {
...
$this->renderer->assign('bpshj', "人員配置初期値");
$this->renderer->assign('bpshe', "AdminJinin");
